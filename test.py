...
def test_tensor(browser):
    ...
    assert sbis_page.driver.current_url=='https://tensor.ru/about', 'Попадаем не в https://tensor.ru/about'
...
def test_partners_regions(browser):
    ...
    assert sbis_page.is_contacts_present()!=False, 'Отсутствуют контакты партнеров'
    sbis_page.go_to_choose_region()
    assert sbis_page.check_region_kamchatka()==True, 'Не поменялся регион на Камчатский край'
    # assert sbis_page.is_region_correct()=='Камчатский край', 'Не поменялся регион на Камчатский край'
    assert sbis_page.is_contacts_present()!=False, 'Отсутствуют партнеры после смены региона'
    current_url=sbis_page.driver.current_url
    assert '41-kamchatskij-kraj' in current_url, 'Некорректный url'
    assert sbis_page.check_region_kamchatka_in_title()==True
    # assert 'Камчатский' in sbis_page.driver.title, 'Отсутствует информация о регионе в заголовке'
def test_plugin(browser_with_option):
    ...
    time.sleep(2)
    assert sbis_page.check_plugin_size!=False, 'Error'